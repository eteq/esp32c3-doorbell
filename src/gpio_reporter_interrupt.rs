...
fn main() -> ! {
    ...
    let wifi_stack = setup_wifi(VERBOSE, wifi_timer, wifi_rng, system.radio_clock_control, &clocks, radio_wifi, &mut wifi_socket_set_entries).unwrap();
    ...
    let mut push_counter = rb.capacity();
    loop {
        ...
        while (SystemTimer::now() - start_time) < SENSE_WAIT_TIME_TICKS {
            ...
            fall_time = critical_section::with(|cs| {
            ...
            if fall_time != None {
                fall_time = Some(fall_time.unwrap() - start_time);
                break;
            }
        }
        if fall_time != None {
            rb.push_overwrite(fall_time.unwrap() as u32);
            push_counter -= 1;
        }
        if push_counter <= 0 {
            ...
            socket = send_rb(socket, &rb);

            let wait_end = SystemTimer::now() + SystemTimer::TICKS_PER_SECOND/5; // 200 ms
...
fn send_rb<'a>(mut socket: Socket<'a, 'a>, rb: & StaticRb<u32, BUFFER_SIZE>) -> Socket<'a, 'a>{
    socket.open(IpAddress::Ipv4(SEND_ADDRESS), SEND_PORT).unwrap();

    let mut data_s: heapless::String<{10*BUFFER_SIZE}> = heapless::String::new();
    for elem in rb.iter() {
        use core::fmt::Write;
        write!(data_s, "{},", elem).unwrap();
    }
    if data_s.len() > 0 { data_s.pop(); }
            ...
            match res {
                ...
                Err(err) => {
                    ...
                    if nerr < N_RETRY_CONNECT {
                        if verbose {
                            println!("Got on error on try {} while trying to connect: {:?}", nerr, err);
                        }
                        break 'inner;
                    } else {
                        return Err(err);
